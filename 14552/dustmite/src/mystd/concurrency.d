 ...
 */
module mystd.concurrency;


public
{
    import mystd.variant;
}
private
{
    import core.atomic;
    ...
    template hasLocalAliasing(T...)
    {
        static if( !T.length )
            ...
            enum hasLocalAliasing = (mystd.traits.hasUnsharedAliasing!(T[0]) && !is(T[0] == Tid)) ||
                                     mystd.concurrency.hasLocalAliasing!(T[1 .. $]);
    }
 ...
 * Example:
 * ---
 * import mystd.stdio, mystd.concurrency;
 *
 * void f1(string str)
 * {
 *     writeln(str);
 ...
 * user-level option, mystd.concurrency may be used for far more types of
