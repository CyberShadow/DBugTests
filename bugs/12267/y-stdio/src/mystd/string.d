...
import core.vararg, core.stdc.stdlib, core.stdc.string,
    mystd.algorithm, mystd.ascii, mystd.conv, mystd.exception, mystd.format, mystd.functional,
    mystd.range, mystd.traits,
    mystd.typecons, mystd.typetuple, mystd.uni, mystd.utf;

//Remove when repeat is finally removed. They're only here as part of the
//deprecation of these functions in mystd.string.
public import mystd.algorithm : startsWith, endsWith, cmp, count;
public import mystd.array : join, split;
...
version(unittest) import mystd.algorithm : filter;
    ...
    if (isSomeChar!Char)
{
    ...
    else
    {
        ...
        {                                                   // Plain old ASCII
            ...
            {
                auto c3 = mystd.ascii.toLower(c2);
                if (c1 == c3)
                    return i;
            }
        }
...
/++
  ...
  +/
alias toLower = mystd.uni.toLower;
/++
 ...
 +/
alias toLowerInPlace = mystd.uni.toLowerInPlace;
...
/++
    ...
    If none of $(D s) characters were affected, then $(D s) itself is returned.
  +/
alias toUpper = mystd.uni.toUpper;
...
/++
 ...
 +/
alias toUpperInPlace = mystd.uni.toUpperInPlace;
...
unittest
{
    debug(string) printf("mystd.string.format.unittest\n");
...
char[] sformat(Char, Args...)(char[] buf, in Char[] fmt, Args args)
{
    ...
    struct Sink
    {
        void put(dchar c)
        {
            ...
            if (buf.length < i + n)
                onRangeError("mystd.string.sformat", 0);
...
unittest
{
    debug(string) printf("mystd.string.sformat.unittest\n");
...
deprecated("Please use mystd.string.format instead.") alias xformat = format;

deprecated unittest
{
    debug(string) printf("mystd.string.xformat.unittest\n");
...
// Explicitly undocumented. It will be removed in July 2014.
deprecated("Please use mystd.string.sformat instead.") alias xsformat = sformat;

deprecated unittest
{
    debug(string) printf("mystd.string.xsformat.unittest\n");
...
unittest
{
    debug(string) printf("mystd.string.inPattern.unittest\n");
...
unittest
{
    debug(string) printf("mystd.string.count.unittest\n");
...
unittest
{
    debug(string) printf("mystd.string.removechars.unittest\n");
...
unittest
{
    debug(string) printf("mystd.string.squeeze.unittest\n");
...
unittest
{
    debug(string) printf("mystd.string.succ.unittest\n");
       ...
       (C1[] str, const(C2)[] from, const(C3)[] to, const(C4)[] modifiers = null)
{
    ...
    if (to.empty && !mod_d)
        to = mystd.conv.to!(typeof(to))(from);
...
unittest
{
    debug(string) printf("mystd.string.tr.unittest\n");
...
S[] outdent(S)(S[] lines) @safe pure if(isSomeString!S)
{
    ...
    {
        return str[ 0 .. $-find!(not!(mystd.uni.isWhite))(str).length ];
    }
...
// TODO: Remove this and use mystd.string.strip when retro() becomes ctfe-able.
