...
unittest
{
    ...
    import mystd.digest.crc, mystd.digest.sha, mystd.digest.md;
    import mystd.stdio;
...
unittest
{
    ...
    void digestFile(Hash)(ref Hash hash, string filename) if(isDigest!Hash)
    {
        ...
        //As digests imlement OutputRange, we could use mystd.algorithm.copy
...
unittest
{
    ...
    void digestFile(Digest hash, string filename)
    {
        ...
        //As digests implement OutputRange, we could use mystd.algorithm.copy
        //Let's do it manually for now
        foreach (buffer; file.byChunk(4096 * 1024))
          hash.put(buffer);
...
template isDigest(T)
{
    import mystd.range : isOutputRange;
...
unittest
{
    import mystd.digest.crc, mystd.digest.md;
    assert(!hasPeek!(MD5));
...
{
    import mystd.digest.md;
    import mystd.range : isInputRange, ElementType;
...
unittest
{
    import mystd.digest.md, mystd.digest.sha, mystd.digest.crc;
...
(in ubyte[num] digest)
{
    ...
    {
        import mystd.ascii : hexDigits = hexDigits;
    }
    else
    {
        import mystd.ascii : hexDigits = lowerHexDigits;
    }
