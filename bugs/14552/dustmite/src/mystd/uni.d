...
/++
    ...
    and ignore Unicode $(CHARACTERS), see $(LINK2 std_ascii.html, mystd.ascii).
...
debug(std_uni) import mystd.stdio;
...
{
    import mystd.internal.unicode_tables; // generated file
}
...
{
    import mystd.algorithm : sum, map;
...
//@@@BUG: mystd.typecons tuples depend on mystd.format to produce fields mixin
// which relies on mystd.uni.isGraphical and this chain blows up with Forward reference error
...
/**
    ...
    or the exact type of a current $(CODEPOINT) set in $(D mystd.uni).
...
{
    import mystd.range : assumeSorted;
...
public:
    ...
    */
    ...
    /**
        ...
        Example:
        ---
        import mystd.algorithm, mystd.typecons;
    ...
    */
    ...
    // TODO:
    // used internally in mystd.regex
...
public:
    ...
    */
    ...
    private import mystd.format : FormatException, FormatSpec;
...
private:
  ...
  package(std)  // used from: mystd.regex.internal.parser
  ...
  package(std)  // used from: mystd.regex.internal.parse
    ref add(U)(U rhs)
        if(isCodepointSet!U)
    {
        Marker start;
...
private:
    ...
    static struct Intervals(Range)
    {
        ...
        //may break sorted property - but we need mystd.sort to access it
    ...
    void sanitize()
    {
        import mystd.algorithm : sort, SwapStrategy, max;
        if (data.length == 0)
            return;
    ...
    body
    {
        import mystd.range : assumeSorted, SearchPolicy;
...
@trusted struct CowArray(SP=GcPolicy)
{
    ...
    {
        import mystd.algorithm : min, copy;
        if(len == 0)
        {
            if(!empty)
                freeThisReference();
...
@safe unittest
{
    import mystd.conv, mystd.range, mystd.algorithm;
    ...
    */
    ...
    auto buildTrie(Range)(Range range, Value filler, bool unsorted)
        if(isInputRange!Range
            ...
            && is(typeof(Range.init.front[1]) : Key))
    {
        import mystd.algorithm : multiSort;
...
@safe unittest
{
    ...
    foreach(ch; unicode.L.byCodepoint.stride(3))
    {
        ...
        char[4] buf;
        wchar[2] buf16;
        auto len = mystd.utf.encode(buf, ch);
        auto len16 = mystd.utf.encode(buf16, ch);
...
/**
    ...
    for algorithms like mystd.algorithm.find that take unary predicates. )
...
{
    import mystd.ascii : toLower;
    import mystd.algorithm : cmp, map, filter;
    ...
    && isForwardRange!S2 && is(Unqual!(ElementType!S2) == dchar))
{
    ...
    foreach(dchar lhs; str1)
    {
        if(ridx == str2.length)
            return 1;
        dchar rhs = mystd.utf.decode(str2, ridx);
        int diff = lhs - rhs;
        if(!diff)
            continue;
...
{
    import mystd.algorithm : skipOver;
...
// This is package for the moment to be used as a support tool for mystd.regex
...
{
    import mystd.internal.unicode_comp;
    import mystd.algorithm : map;
    import mystd.range : assumeSorted;
...
{
    import mystd.internal.unicode_decomp;
    import mystd.algorithm : until;
...
{
    import mystd.algorithm : sort, SwapStrategy;
    import mystd.range : zip;
...
private auto seekStable(NormalizationForm norm, C)(size_t idx, in C[] input)
{
    ...
    for(;;)
    {
        ...
        {
            region_start = br.length - mystd.utf.codeLength!C(ch);
            break;
        }
        br.popFront();
    }
...
{
    import mystd.ascii : isLower, isASCII;
    if(isASCII(c))
        return isLower(c);
...
{
    import mystd.ascii : isUpper, isASCII;
    if(isASCII(c))
        return isUpper(c);
...
// TODO: helper, I wish mystd.utf was more flexible (and stright)
...
@trusted unittest //@@@BUG mystd.format is not @safe
{
    import mystd.format : format;
    foreach(ch; 0..0x80)
        assert(mystd.ascii.toLower(ch) == toLower(ch));
    assert(toLower('Я') == 'я');
    assert(toLower('Δ') == 'δ');
...
@trusted unittest
{
    import mystd.format : format;
    foreach(ch; 0..0x80)
        assert(mystd.ascii.toUpper(ch) == toUpper(ch));
    assert(toUpper('я') == 'Я');
    assert(toUpper('δ') == 'Δ');
...
@safe pure nothrow @nogc @property
{
    ...
    auto nfcQCTrie()
    {
        import mystd.internal.unicode_norm;
    ...
    auto mcTrie()
    {
        import mystd.internal.unicode_grapheme;
