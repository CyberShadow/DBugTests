module mystd.experimental.logger.core;
...
import mystd.experimental.logger.filelogger;
...
$(LREF mystd.datatime.DateTime.toISOExtString) expect the fractional second part.
...
{
    import mystd.traits : isSomeString, isSomeChar;
...
private void formatString(A...)(MsgRange oRange, A args)
{
    ...
    foreach (arg; args)
    {
        mystd.format.formattedWrite(oRange, "%s", arg);
    }
}
...
abstract class Logger
{
    ...
    */
    ...
    /* The default implementation will use an $(D mystd.array.appender)
...
@safe unittest
{
    ...
    testFuncNames(tl1);
    assert(tl1.func == "mystd.experimental.logger.core.testFuncNames", tl1.func);
    assert(tl1.prettyFunc ==
        "void mystd.experimental.logger.core.testFuncNames(Logger logger) @safe",
        tl1.prettyFunc);
...
@safe unittest
{
    import mystd.experimental.logger.multilogger : MultiLogger;
...
unittest
{
    import mystd.concurrency, core.atomic, core.thread;
