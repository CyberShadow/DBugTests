 ...
 *
 * The mystd.cstream module bridges mystd.c.stdio (or mystd.stdio) and mystd.stream.
 * Both mystd.c.stdio and mystd.stream are publicly imported by mystd.cstream.
 *
 * Macros:
 ...
 */
module mystd.cstream;

public import mystd.stream;
public import mystd.c.stdio;
...
class CFile : Stream {
  ...
  override char ungetc(char c) {
    return cast(char)mystd.c.stdio.ungetc(c,cfile);
  }
...
/**
 * CFile wrapper of mystd.c.stdio.stdin (not seekable).
 */
__gshared CFile din;

/**
 * CFile wrapper of mystd.c.stdio.stdout (not seekable).
 */
__gshared CFile dout;

/**
 * CFile wrapper of mystd.c.stdio.stderr (not seekable).
 */
__gshared CFile derr;

shared static this() {
  // open standard I/O devices
  din = new CFile(mystd.c.stdio.stdin,FileMode.In);
  dout = new CFile(mystd.c.stdio.stdout,FileMode.Out);
  derr = new CFile(mystd.c.stdio.stderr,FileMode.Out);
}
