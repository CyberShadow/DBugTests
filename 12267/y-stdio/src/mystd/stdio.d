...
/**
Standard I/O functions that extend $(B mystd.c.stdio).  $(B mystd.c.stdio)
...
public import core.stdc.stdio, mystd.string : KeepTerminator;
static import mystd.c.stdio;
...
import mystd.range;
import mystd.traits : Unqual, isSomeChar, isAggregateType, isSomeString,
...
version (DigitalMars)
{
    version (Win32)
    {
        ...
        import mystd.c.stdio : __fhnd_info, FHND_WCHAR, FHND_TEXT;
    }
    else version (Win64)
    {
 ...
 */
    void close()
    {
        ...
        //fprintf(mystd.c.stdio.stderr, ("Closing file `"~name~"`.\n\0").ptr);
 ...
 */
    ...
    unittest
    {
        assert(stdout.getFP() == mystd.c.stdio.stdout);
    }
...
/// ditto
    ...
    {
        static import mystd.file;
        import mystd.algorithm : take, equal;
...
void writefx(FILE* fps, TypeInfo[] arguments, void* argptr, int newline=false)
{
    ...
    if (orientation <= 0)                // byte orientation or no orientation
    {
        void putc(dchar c)
        {
            ...
            {
                char[4] buf = void;
                foreach (i; 0 .. mystd.utf.toUTF8(buf, c).length)
                    FPUTC(buf[i], fp);
            }
        }

        mystd.format.doFormat(&putc, arguments, argptr);
        if (newline)
            FPUTC('\n', fp);
    }
    else if (orientation > 0)                // wide orientation
    {
        ...
        mystd.format.doFormat(&putcw, arguments, argptr);
        if (newline)
            FPUTWC('\n', fp);
    }
}
...
{
    /***********************************
     * Convenience function that forwards to $(D mystd.c.stdio.popen)
     * with appropriately-constructed C-style strings.
     */
...
/*
 * Convenience function that forwards to $(D mystd.c.stdio.fwrite)
...
Initialize with a message and an error code. */
    this(string message, uint e = .errno)
    {
        ...
        version (Posix)
        {
            import mystd.c.string : strerror_r;

            char[256] buf = void;
            version (linux)
            {
                auto s = mystd.c.string.strerror_r(errno, buf.ptr, buf.length);
            }
            else
            {
                mystd.c.string.strerror_r(errno, buf.ptr, buf.length);
                auto s = buf.ptr;
            }
        }
        else
...
version(linux)
{
    static import linux = mystd.c.linux.linux;
    static import sock = mystd.c.linux.socket;
    ...
    File openNetwork(string host, ushort port)
    {
        ...
        auto h = enforce( sock.gethostbyname(mystd.string.toStringz(host)),
