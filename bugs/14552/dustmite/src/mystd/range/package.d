...
mystd.algorithm)) to be used with a vast variety of different concrete types. For
example, a linear search algorithm such as $(LINK2 std_algorithm.html#find,
mystd.algorithm.find) works not just for arrays, but for linked-lists, input
...
$(LIST
$(DIV ,
The $(LINK2 std_range_primitives.html, $(D mystd._range.primitives)) submodule
...
$(DIV ,
The $(LINK2 std_range_interfaces.html, $(D mystd._range.interfaces)) submodule
...
std_algorithm.html#sort, $(D mystd.algorithm.sort)) function also conveniently
...
public import mystd.range.primitives;
public import mystd.range.interfaces;
public import mystd.array;
public import mystd.typecons : Flag, Yes, No;
...
@safe unittest
{
    import mystd.internal.test.dummyrange;
...
// TODO: use a vtable (or more) instead of linear iteration
        ...
        public:
            ...
            import mystd.typetuple : anySatisfy;
...
@safe unittest
{
    import mystd.algorithm : equal, filter;
...
@safe unittest
{
    import mystd.algorithm : equal, filterBidirectional;
...
@safe pure unittest
{
    import mystd.algorithm : equal, map;
...
{
    import mystd.format : format; //for generic mixins
    ...
    static if (allSatisfy!(hasLength, R))
    {
        @property auto length()
        {
            ...
            else
            {
                ...
                import mystd.algorithm : min, max;
...
{
    import mystd.internal.test.dummyrange;
    import mystd.algorithm : swap, sort, filter, equal, map;

    import mystd.exception : assertThrown, assertNotThrown;
    import mystd.typecons : tuple;
...
{
    private import mystd.functional : binaryFun;
...
@safe unittest
{
    import mystd.math : pow, round, sqrt;
...
/**
   ...
   range.
    ...
    void main()
    {
        ...
        import mystd.algorithm : map, copy;
        import mystd.format;
...
nothrow @nogc unittest
{
    ...
    auto t1 = iota(0, 10, 2);
    auto t2 = iota(1, 1, 0);
    //float overloads use mystd.conv.to so can't be @nogc or nothrow
}
...
@safe unittest
{
    import mystd.math : approxEqual, nextUp, nextDown;
    import mystd.algorithm : count, equal;
...
unittest
{
    import mystd.algorithm.comparison : equal;
    import mystd.bigint;
        ...
        hasAssignableElements!RangeOfRanges)
{
    ...
    @property auto front()
    {
        import mystd.algorithm : filter, map;
        return _input.save
                     .filter!(a => !a.empty)
                     .map!(a => a.front);
    }
...
@safe unittest
{
    import mystd.algorithm : equal, joiner;
...
----
    import mystd.stdio : stdin, stdout;
    import mystd.range : enumerate;
...
in
{
    static if (hasLength!Range)
    {
        ...
        import mystd.conv : ConvException, to;
        import core.exception : RangeError;
...
version(none) // @@@BUG@@@ 10939
{
    ...
    /+pure+/ unittest // Impure because of mystd.conv.to
    {
        import core.exception : RangeError;
        import mystd.exception : assertNotThrown, assertThrown;
...
if (isInputRange!Range)
{
    ...
    private void dbgVerifySorted()
    {
        ...
        debug
        {
            ...
            import mystd.conv : text;
            import mystd.random : MinstdRand, uniform;

            static if (isRandomAccessRange!Range)
            {
                import mystd.algorithm : isSorted;
 ...
 */
    auto groupBy()()
    {
        import mystd.algorithm.iteration : chunkBy;
...
unittest
{
    import mystd.stdio, mystd.file, mystd.path, mystd.conv, mystd.uuid;
    auto name = buildPath(tempDir(), "test.mystd.range.line-" ~ text(__LINE__) ~
                          "." ~ randomUUID().toString());
    auto f = File(name, "w");
...
{
    import mystd.algorithm : map, joiner, group, until;
    // fix for mystd.algorithm
    auto r = map!(x => 0)([1]);
    chain(r, r);
    zip(r, r);
    roundRobin(r, r);
...
@safe unittest
{
    import mystd.algorithm : equal, filter, map;
...
@safe unittest
{
    import mystd.algorithm : filter, equal, map;

    char[] txt = "Line one, Line 2".dup;

    bool isVowel(dchar c)
    {
        return mystd.string.indexOf("AaEeIiOoUu", c) != -1;
    }
