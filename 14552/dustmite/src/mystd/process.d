    ...
    @trusted // TODO: Should be @safe
{
    ...
    import mystd.path : isDirSeparator;
    import mystd.algorithm : any;
...
{
    import mystd.algorithm : splitter;
...
unittest // Reopening the standard streams (issue 13258)
{
    ...
    {
        auto oldOut = mystd.stdio.stdout;
        scope(exit) mystd.stdio.stdout = oldOut;

        mystd.stdio.stdout = File(tmpFile, "w");
        fun();
        mystd.stdio.stdout.close();
    }
...
unittest // MSVCRT workaround (issue 14422)
{
    ...
    mystd.file.write(fn, "AAAAAAAAAA");

    auto f = File(fn, "a");
    spawnProcess(["cmd", "/c", "echo BBBBB"], mystd.stdio.stdin, f).wait();
...
Returns:
An $(D mystd.typecons.Tuple!(bool, "terminated", int, "status")).
...
Returns:
An $(D mystd.typecons.Tuple!(int, "status", string, "output")).
