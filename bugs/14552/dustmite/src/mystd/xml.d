...
void main()
{
    string s = cast(string)mystd.file.read("books.xml");
...
import mystd.algorithm : count, startsWith;
import mystd.array;
import mystd.ascii;
import mystd.string;
 ...
 * the mystd.xml classes, because then all the encoding and decoding will be done
        ...
        else
        {
            ...
            if (startsWith(s[i..$],"&#"))
            {
                try
                {
                    ...
                    checkCharRef(t, d);
                    char[4] temp;
                    buffer ~= temp[0 .. mystd.utf.encode(temp, d)];
...
private
{
    ...
    {
        // TO DO - Fix mystd.utf stride and decode functions, then use those
        // instead
