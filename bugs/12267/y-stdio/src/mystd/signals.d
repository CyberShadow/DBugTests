 ...
 */
module mystd.signals;

import mystd.stdio;
import mystd.c.stdlib : calloc, realloc, free;
...
mixin template Signal(T1...)
{
    ...
    final void connect(slot_t slot)
    {
        ...
        if (slots_idx == len)
        {
            ...
            {
                len = 4;
                auto p = mystd.c.stdlib.calloc(slot_t.sizeof, len);
                if (!p)
            ...
            {
                len = len * 2 + 4;
                auto p = mystd.c.stdlib.realloc(slots.ptr, slot_t.sizeof * len);
                if (!p)
                    core.exception.onOutOfMemoryError();
                slots = (cast(slot_t*)p)[0 .. len];
