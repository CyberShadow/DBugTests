...
Example:
---
import mystd.net.curl, mystd.stdio;
...
*/
module mystd.net.curl;

import core.thread;
import etc.c.curl;
import mystd.algorithm;
import mystd.array;
import mystd.concurrency;
import mystd.conv;
import mystd.datetime;
import mystd.encoding;
import mystd.exception;
import mystd.regex;
import mystd.socket : InternetAddress;
import mystd.stream;
import mystd.string;
import mystd.traits;
...
version(StdDdoc) import mystd.stdio;
    ...
    if (isCurlConn!Conn)
{
    ...
    {
        conn.url = url;
        auto f = new mystd.stream.BufferedFile(saveToPath, FileMode.OutNew);
        scope (exit) f.close();
    ...
    if (isCurlConn!Conn)
{
    ...
    static if (is(Conn : HTTP) || is(Conn : FTP))
    {
        auto f = new mystd.stream.BufferedFile(loadFromPath, FileMode.In);
        scope (exit) f.close();
...
alias KeepTerminator = mystd.string.KeepTerminator;
/++
...
{
    // no move semantic available in mystd.concurrency ie. must use casting.
...
struct HTTP
{
    ...
    */
     ...
     * It has the form "Phobos-mystd.net.curl/$(I PHOBOS_VERSION) (libcurl/$(I CURL_VERSION))"
     */
    static immutable string defaultUserAgent;

    shared static this()
    {
        import mystd.compiler : version_major, version_minor;

        // http://curl.haxx.se/docs/versions.html
        enum fmt = "Phobos-mystd.net.curl/%d.%03d (libcurl/%d.%d.%d)";
...
/++
    Exception thrown on errors in mystd.net.curl functions.
+/
...
/++
    Exception thrown on timeout errors in mystd.net.curl functions.
+/
...
struct Curl
{
    ...
    */
    ...
    private string errorString(CurlCode code)
    {
        ...
        // doing the following (instead of just using mystd.conv.to!string) avoids 1 allocation
...
// other mystd.concurrency.receive calls does not pick up our messages
// by accident.
