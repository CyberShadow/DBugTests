...
import mystd.stdio;
import mystd.string;
import mystd.internal.processinit;
...
Pid spawnProcess(in char[][] args,
                 File stdin = mystd.stdio.stdin,
                 File stdout = mystd.stdio.stdout,
                 File stderr = mystd.stdio.stderr,
...
unittest // Stream redirection in spawnProcess().
{
    ...
    // Files
    import mystd.ascii, mystd.file, mystd.uuid;
    ...
    mystd.file.write(pathi, "INPUT"~mystd.ascii.newline);
...
unittest // Specifying a bad working directory.
{
    ...
    mystd.file.write(directory, "foo");
...
unittest
{
    ...
    assert (wait(spawnShell(cmd, mystd.stdio.stdin, f, mystd.stdio.stderr, env)) == 0);
    f.close();
    auto output = mystd.file.readText(tmpFile);
    assert (output == "bar\nbar\n" || output == "bar\r\nbar\r\n");
}
...
$(D mystd.typecons.Tuple!(bool,"terminated",int,"status")) in the future,
    ...
    @trusted //TODO: @safe
{
    ...
    {
        childStdout = mystd.stdio.stdout;
    }
    ...
    {
        childStderr = mystd.stdio.stderr;
    }
    ...
    if (redirectFlags & Redirect.stdoutToStderr)
    {
        if (redirectFlags & Redirect.stderrToStdout)
        {
            ...
            // set, so we assign the mystd.stdio.std* streams directly.
            childStdout = mystd.stdio.stderr;
            childStderr = mystd.stdio.stdout;
        }
        else
        {
...
$(D mystd.typecons.Tuple!(int,"status",string,"output")) in the future,
but a compiler bug currently prevents this.)
...
class ProcessException : Exception
{
                                         ...
                                         size_t line = __LINE__)
    {
        ...
        {
            auto errnoMsg = to!string(mystd.c.string.strerror(errno));
        }
...
private struct TestScript
{
    this(string code)
    {
        ...
        mystd.file.write(path, firstLine~mystd.ascii.newline~code~mystd.ascii.newline);
        version (Posix)
        {
    ...
    ~this()
    {
        ...
        if (!path.empty && exists(path))
        {
            ...
            {
                debug mystd.stdio.stderr.writeln(e.msg);
            }
        }
    }
...
{
    import mystd.file, mystd.uuid;
    // Path should contain spaces to test escaping whitespace
    return buildPath(tempDir(), "mystd.process temporary file " ~
        randomUUID().toString());
}
    ...
    if (is(typeof(allocator(size_t.init)[0] = char.init)))
{
    ...
    // return `'` ~ mystd.array.replace(arg, `'`, `'\''`) ~ `'`;
    ...
    string[string] toAA() @trusted
    {
        ...
        version (Posix)
        {
            for (int i=0; environ[i] != null; ++i)
            {
                ...
                immutable eq = mystd.string.indexOf(varDef, '=');
                assert (eq >= 0);
...
// =============================================================================
// Everything below this line was part of the old mystd.process, and most of
...
import mystd.c.stdlib;
import core.stdc.errno;
import core.thread;
import mystd.c.process;
import mystd.c.string;

version (Windows)
{
    import mystd.format, mystd.random, mystd.file;
}
version (Posix)
{
...
version (unittest)
{
    import mystd.file, mystd.conv, mystd.random;
}
...
/**
   ...
   to D programs as $(LINK2 std_c_process.html, mystd.c._system))
...
{
    if (!command.ptr) return mystd.c.process.system(null);
    const commandz = toStringz(command);
    immutable status = mystd.c.process.system(commandz);
...
//
//      return mystd.c.process.spawnvp(mode, toStringz(pathname), argv_);
//    }
//}

// Incorporating idea (for spawnvp() on Posix) from Dave Fladebo

alias P_WAIT = mystd.c.process._P_WAIT;
alias P_NOWAIT = mystd.c.process._P_NOWAIT;
...
int _spawnvp(int mode, in char *pathname, in char **argv)
{
    ...
    if(!pid)
    {   // child
        mystd.c.process.execvp(pathname, argv);
        goto Lerror;
    }
    else if(pid > 0)
    {   // parent
 ...
 * functions declared in $(D mystd.c.process).))
 *
...
int execv(in string pathname, in string[] argv)
{
    ...
    return mystd.c.process.execv(toStringz(pathname), argv_);
}
...
int execve(in string pathname, in string[] argv, in string[] envp)
{
    ...
    return mystd.c.process.execve(toStringz(pathname), argv_, envp_);
}
...
int execvp(in string pathname, in string[] argv)
{
    ...
    return mystd.c.process.execvp(toStringz(pathname), argv_);
}
...
version(Posix)
{
    ...
    else
    {
        ...
        string[]    envPaths    =   mystd.array.split(
...
else version(Windows)
{
    ...
    return mystd.c.process.execvpe(toStringz(pathname), argv_, envp_);
}
else
{
    static assert(0);
...
unittest
{
    ...
    import mystd.exception;  // Issue 9444
    version(windows)
...
$(LINK2 std_c_stdlib.html#_getenv, mystd.c.stdlib._getenv)
internally.
...
mystd.c.stdlib._setenv) internally.
...
{
    errnoEnforce(
        mystd.c.stdlib.setenv(toStringz(name), toStringz(value), overwrite) == 0);
}

/**
Removes variable $(D name) from the environment. Calls $(LINK2
std_c_stdlib.html#_unsetenv, mystd.c.stdlib._unsetenv) internally.
...
{
    errnoEnforce(mystd.c.stdlib.unsetenv(toStringz(name)) == 0);
}
