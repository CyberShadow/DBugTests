...
// Petit demangler
// (this or similar thing will eventually go to mystd.demangle if necessary
...
unittest
{
    ...
    // static assert(packageName!std == "std");  // this package (currently: "mystd.std")
    static assert(packageName!(mystd.traits) == "std");     // this module
    static assert(packageName!packageName == "std");      // symbol in this module
    static assert(packageName!(mystd.algorithm) == "std");  // other module from same package
 ...
 * ---
 * import mystd.traits;
 * static assert(moduleName!moduleName == "mystd.traits");
 * ---
 */
...
unittest
{
    ...
    static assert(moduleName!(mystd.traits) == "mystd.traits");            // this module
    static assert(moduleName!moduleName == "mystd.traits");              // symbol in this module
    static assert(moduleName!(mystd.algorithm) == "mystd.algorithm");      // other module
    static assert(moduleName!(mystd.algorithm.map) == "mystd.algorithm");  // symbol in other module
 ...
 * static assert(fullyQualifiedName!fullyQualifiedName == "mystd.traits.fullyQualifiedName");
 * ---
 */
...
private template fullyQualifiedNameImplForSymbols(alias T)
{
    ...
    {
        import mystd.algorithm : skipOver, findSplit;
...
unittest
{
    ...
    static assert(fqn!fqn == "mystd.traits.fullyQualifiedName");
    static assert(fqn!(QualifiedNameTests.Inner) == "mystd.traits.QualifiedNameTests.Inner");
    static assert(fqn!(QualifiedNameTests.func) == "mystd.traits.QualifiedNameTests.func");
...
unittest
{
    ...
    // Main tests
    alias fqn = fullyQualifiedName;
    enum inner_name = "mystd.traits.QualifiedNameTests.Inner";
    with (QualifiedNameTests)
    {
        ...
        static assert(fqn!(QualifiedEnum) == "mystd.traits.QualifiedEnum"); // type
        static assert(fqn!(QualifiedEnum.a) == "mystd.traits.QualifiedEnum.a"); // symbol
...
template RepresentationTypeTuple(T)
{
    template Impl(T...)
    {
        ...
        {
            import mystd.typecons : Rebindable;
 ...
 * Example:
 * ---
 * import mystd.traits, mystd.typetuple, mystd.stdio;
 * interface I { }
 * class A { }
 * class B : A, I { }
 *
...
unittest
{
    ...
    // from mystd.conv.implicitlyConverts
...
$(D mystd.range.ElementType!Range) in the case of narrow strings, or if T
...
deprecated("unsigned has been moved to mystd.conv. Please adjust your imports accordingly.")
alias unsigned = mystd.conv.unsigned;
...
--------------------
module test;
import mystd.traits : mangledName;
