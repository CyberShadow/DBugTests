 ...
 *   and $(D mystd.getopt.config.passThrough) was not present.
 */
...
---------
 ...
 If $(D mystd.getopt.config.passThrough) was not passed to getopt
...
with the $(D mystd.getopt.config.bundling) directive:
...
bool foo, bar;
getopt(args,
    mystd.getopt.config.required,
    "foo|f", &foo,
    "bar|b", &bar);
---------

Only the option direclty following $(D mystd.getopt.config.required) is
required.
...
{
    import mystd.algorithm : map, splitter;
    import mystd.ascii : isAlpha;
    import mystd.conv : text, to;
    ...
    for (size_t i = 1; i < args.length; )
    {
        ...
        else
        {
            ...
            {
                // array receiver
                import mystd.range : ElementEncodingType;
            ...
            else static if (isAssociativeArray!(typeof(*receiver)))
            {
                ...
                import mystd.range : only;
                import mystd.typecons : Tuple, tuple;
                import mystd.string : indexOf;
...
{
    import mystd.bitmanip : bitfields;
    mixin(bitfields!(
...
unittest
{
    ...
    getopt(args,
        mystd.getopt.config.keepEndOfOptions,
        "foo", &foo,
        "bar", &bar);
...
// Issue 13316 - mystd.getopt: implicit help option breaks the next argument
unittest
{
...
// Issue 13317 - mystd.getopt: endOfOptions broken when it doesn't look like an option
unittest
{
