...
to module $(LINK2 std_stdio.html,$(D mystd.stdio)).
...
import core.stdc.stdio, core.stdc.stdlib, core.stdc.string,
       core.stdc.errno, mystd.algorithm, mystd.array, mystd.conv,
       mystd.datetime, mystd.exception, mystd.format, mystd.path, mystd.process,
       mystd.range, mystd.stdio, mystd.string, mystd.traits,
       mystd.typecons, mystd.typetuple, mystd.utf;


version (Windows)
{
    import core.sys.windows.windows, mystd.windows.syserror;
}
else version (Posix)
{
    ...
    version(Windows)
    {
        alias defaults =
            TypeTuple!(GENERIC_READ,
                ...
                HANDLE.init);
        auto h = CreateFileW(mystd.utf.toUTF16z(name), defaults);
...
version (linux) unittest
{
    ...
    auto s = mystd.file.readText("/proc/sys/kernel/osrelease");
    assert(s.length > 0);
    //writefln("'%s'", s);
}
...
void rename(in char[] from, in char[] to)
{
    version(Windows)
    {
        enforce(MoveFileExW(mystd.utf.toUTF16z(from), mystd.utf.toUTF16z(to), MOVEFILE_REPLACE_EXISTING),
                new FileException(
...
void remove(in char[] name)
{
    version(Windows)
    {
        cenforce(DeleteFileW(mystd.utf.toUTF16z(name)), name);
    }
    else version(Posix)
...
version(Windows) private WIN32_FILE_ATTRIBUTE_DATA getFileAttributesWin(in char[] name)
{
    ...
    enforce(GetFileAttributesExW(mystd.utf.toUTF16z(name), GET_FILEEX_INFO_LEVELS.GetFileExInfoStandard, &fad), new FileException(name.idup));
    return fad;
}
              ...
              out SysTime modificationTime)
{
    version(Windows)
    {
        with (getFileAttributesWin(name))
        {
            accessTime = mystd.datetime.FILETIMEToSysTime(&ftLastAccessTime);
            modificationTime = mystd.datetime.FILETIMEToSysTime(&ftLastWriteTime);
        }
    }
    else version(Posix)
    {
                                       ...
                                       out SysTime fileModificationTime)
{
    with (getFileAttributesWin(name))
    {
        fileCreationTime = mystd.datetime.FILETIMEToSysTime(&ftCreationTime);
        fileAccessTime = mystd.datetime.FILETIMEToSysTime(&ftLastAccessTime);
        fileModificationTime = mystd.datetime.FILETIMEToSysTime(&ftLastWriteTime);
    }
}
...
unittest
{
    //mystd.process.system("echo a > deleteme") == 0 || assert(false);
    if(exists(deleteme))
        remove(deleteme);
...
// fileio/base/getfileattributes.asp
        return GetFileAttributesW(mystd.utf.toUTF16z(name)) != 0xFFFFFFFF;
    }
    else version(Posix)
    {
        /*
...
uint getAttributes(in char[] name)
{
    version(Windows)
    {
        immutable result = GetFileAttributesW(mystd.utf.toUTF16z(name));
...
void setAttributes(in char[] name, uint attributes)
{
    version (Windows)
    {
        cenforce(SetFileAttributesW(mystd.utf.toUTF16z(name), attributes), name);
    }
    else version (Posix)
    {
...
void chdir(in char[] pathname)
{
    version(Windows)
    {
        enforce(SetCurrentDirectoryW(mystd.utf.toUTF16z(pathname)),
...
void mkdir(in char[] pathname)
{
    version(Windows)
    {
        enforce(CreateDirectoryW(mystd.utf.toUTF16z(pathname), null),
                new FileException(pathname.idup));
    }
    else version(Posix)
    {
...
private bool ensureDirExists(in char[] pathname)
{
    version(Windows)
    {
        if (CreateDirectoryW(mystd.utf.toUTF16z(pathname), null))
            return true;
...
void rmdir(in char[] pathname)
{
    version(Windows)
    {
        cenforce(RemoveDirectoryW(mystd.utf.toUTF16z(pathname)),
                pathname);
    }
    else version(Posix)
    {
    ...
    public:
        ...
        this(string path)
        {
            ...
            with (getFileAttributesWin(path))
            {
                ...
                _timeCreated = mystd.datetime.FILETIMEToSysTime(&ftCreationTime);
                _timeLastAccessed = mystd.datetime.FILETIMEToSysTime(&ftLastAccessTime);
                _timeLastModified = mystd.datetime.FILETIMEToSysTime(&ftLastWriteTime);
        ...
        private this(string path, in WIN32_FIND_DATA* fd)
        {
            ...
            _name = buildPath(path, mystd.utf.toUTF8(wbuf[0 .. wlength]));
            _size = (cast(ulong)fd.nFileSizeHigh << 32) | fd.nFileSizeLow;
            _timeCreated = mystd.datetime.FILETIMEToSysTime(&fd.ftCreationTime);
            _timeLastAccessed = mystd.datetime.FILETIMEToSysTime(&fd.ftLastAccessTime);
            _timeLastModified = mystd.datetime.FILETIMEToSysTime(&fd.ftLastWriteTime);
            _attributes = fd.dwFileAttributes;
        }
        private this(string path, in WIN32_FIND_DATAW *fd)
        {
            size_t clength = mystd.string.wcslen(fd.cFileName.ptr);
            _name = mystd.utf.toUTF8(fd.cFileName[0 .. clength]);
            _name = buildPath(path, mystd.utf.toUTF8(fd.cFileName[0 .. clength]));
            _size = (cast(ulong)fd.nFileSizeHigh << 32) | fd.nFileSizeLow;
            _timeCreated = mystd.datetime.FILETIMEToSysTime(&fd.ftCreationTime);
            _timeLastAccessed = mystd.datetime.FILETIMEToSysTime(&fd.ftLastAccessTime);
            _timeLastModified = mystd.datetime.FILETIMEToSysTime(&fd.ftLastWriteTime);
...
void copy(in char[] from, in char[] to)
{
    version(Windows)
    {
        immutable result = CopyFileW(mystd.utf.toUTF16z(from), mystd.utf.toUTF16z(to), false);
        if (!result)
    ...
    else version(Posix)
    {
        ...
        scope(failure) core.stdc.stdio.remove(toz);
        {
            ...
            if (!buf)
            {
                ...
                buf || assert(false, "Out of memory in mystd.file.copy");
            }
...
version(Posix) unittest
{
    ...
    mkdirRecurse(d);
    mystd.process.system("ln -sf "~deleteme~"/a/b/c "~deleteme~"/link");
    rmdirRecurse(deleteme);
    enforce(!exists(deleteme));
}
...
private struct DirIteratorImpl
{
    ...
    version(Windows)
    {
        ...
        bool toNext(bool fetch, WIN32_FIND_DATAW* findinfo)
        {
            ...
            while( mystd.string.wcscmp(findinfo.cFileName.ptr, ".") == 0
                    || mystd.string.wcscmp(findinfo.cFileName.ptr, "..") == 0)
                if(FindNextFileW(_stack.data[$-1].h, findinfo) == FALSE)
                {
                    popDirStack();
                    return false;
...
foreach(d; dFiles)
    writeln(d.name);
// Hook it up with mystd.parallelism to compile them all in parallel:
foreach(d; parallel(dFiles, 1)) //passes by 1 file to each thread
{
    string cmd = "dmd -c "  ~ d.name;
    writeln(cmd);
    mystd.process.system(cmd);
}
--------------------
 +/
...
unittest
{
    string testdir = "deleteme.dmd.unittest.mystd.file" ~ to!string(getpid()); // needs to be relative
    ...
    size_t equalEntries(string relpath, SpanMode mode)
    {
        ...
        assert(equal(
                   map!(a => mystd.path.absolutePath(a.name))(dirEntries(relpath, mode)),
