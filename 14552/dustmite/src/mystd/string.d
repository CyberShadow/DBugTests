...
Functions related to Unicode and ASCII are found in $(LINK2 std_uni.html, mystd.uni)
and $(LINK2 std_ascii.html, mystd.ascii), respectively. Other functions that have a
wider generality than just strings can be found in $(LINK2 std_algorithm.html,
mystd.algorithm) and $(LINK2 std_range.html, mystd.range).

See_Also:
    $(LIST
    $(LINK2 std_algorithm.html, mystd.algorithm) and
    $(LINK2 std_range.html, mystd.range)
    for generic range algorithms
    ,
    $(LINK2 std_ascii.html, mystd.ascii)
...
public import mystd.uni : icmp, toLower, toLowerInPlace, toUpper, toUpperInPlace;
public import mystd.format : format, sformat;
...
public import mystd.algorithm : startsWith, endsWith, cmp, count;
public import mystd.array : join, replace, replaceInPlace, split;

/* ************* Exceptions *************** */

/++
    Exception thrown on errors in mystd.string functions.
  +/
...
{
    import mystd.ascii : toLower, isASCII;
    import mystd.uni : toLower;
    import mystd.utf : byDchar, byCodeUnit, UTFException, codeLength;
    ...
    if (cs == CaseSensitive.yes)
    {
        static if (Char.sizeof == 1 && isSomeString!Range)
        {
            ...
            if (mystd.ascii.isASCII(c) && !__ctfe)
            {                                               // Plain old ASCII
    ...
    {
        if (mystd.ascii.isASCII(c))
        {                                                   // Plain old ASCII
            auto c1 = cast(char) mystd.ascii.toLower(c);

            ptrdiff_t i;
            foreach (const c2; s.byCodeUnit())
            {
                if (c1 == mystd.ascii.toLower(c2))
                    return i;
                ++i;
            }
        }
        ...
        {                                                   // c is a universal character
            auto c1 = mystd.uni.toLower(c);

            ptrdiff_t i;
            foreach (const c2; s.byDchar())
            {
                if (c1 == mystd.uni.toLower(c2))
                    return i;
                i += codeLength!Char(c2);
            }
        }
...
@safe pure unittest
{
    ...
    import mystd.utf : byChar, byWchar, byDchar;
    assertCTFEable!(
    {
...
@safe pure unittest
{
    ...
    import mystd.utf : byCodeUnit, byChar, byWchar;
    ...
    if (isForwardRange!Range && isSomeChar!(ElementEncodingType!Range) && isSomeChar!Char)
{
    ...
    {
        import mystd.algorithm : find;

        const(Char1)[] balance;
        if (cs == CaseSensitive.yes)
        {
            balance = mystd.algorithm.find(s, sub);
        }
        else
        {
            balance = mystd.algorithm.find!
                ((a, b) => mystd.uni.toLower(a) == mystd.uni.toLower(b))
                (s, sub);
        }
    ...
    else
    {
        ...
        import mystd.utf : byDchar, codeLength;
...
ptrdiff_t lastIndexOf(Char)(const(Char)[] s, in dchar c,
    ...
    if (isSomeChar!Char)
{
    import mystd.ascii : isASCII, toLower;
    import mystd.utf : canSearchInCodeUnits;
    if (cs == CaseSensitive.yes)
    {
    ...
    else
    {
        if (mystd.ascii.isASCII(c))
        {
            immutable c1 = mystd.ascii.toLower(c);

            foreach_reverse (i, it; s)
            {
                immutable c2 = mystd.ascii.toLower(it);
                if (c1 == c2)
                {
                    return i;
                }
        ...
        {
            immutable c1 = mystd.uni.toLower(c);

            foreach_reverse (i, dchar it; s)
            {
                immutable c2 = mystd.uni.toLower(it);
                if (c1 == c2)
                {
                    return i;
                }
    ...
    if (isSomeChar!Char1 && isSomeChar!Char2)
{
    ...
    else
    {
        for (size_t i = s.length; !s.empty;)
        {
            if (endsWith!((a, b) => mystd.uni.toLower(a) == mystd.uni.toLower(b))
                         (s, sub))
            {
                return cast(ptrdiff_t)i - to!(const(Char1)[])(sub).length;
            }
...
{
    import mystd.algorithm : canFind;
    if (cs == CaseSensitive.yes)
    {
        static if (forward)
        {
            static if (any)
            {
                import mystd.algorithm : findAmong;
    ...
    else
    {
        if (needles.length <= 16 && needles.walkLength(17))
        {
            ...
            {
                scratch[si++] = mystd.uni.toLower(c);
            }
            ...
            static if (forward)
            {
                ...
                {
                    if (canFind(scratch[0 .. si], mystd.uni.toLower(c)) == any)
                    {
                        return i;
                    }
                }
        ...
        else
        {
            ...
            static if (forward)
            {
                foreach (i, dchar c; haystack)
                {
                    if (canFind!f(needles, mystd.uni.toLower(c)) == any)
                    {
                        return i;
                    }
                }
    ...
    if (isSomeString!S)
{
    ...
    foreach (i, dchar c; s)
    {
        ...
        if (i == 0)
        {
            c2 = mystd.uni.toUpper(c);
            if (c != c2)
                changed = true;
        }
        else
        {
            c2 = mystd.uni.toLower(c);
            if (c != c2)
            {
                if (!changed)
                {
...
S[] splitLines(S)(S s, in KeepTerminator keepTerm = KeepTerminator.no) @safe pure
    if (isSomeString!S)
{
    import mystd.uni : lineSep, paraSep;
    import mystd.array : appender;
...
{
    import mystd.ascii : isASCII, isWhite;
    import mystd.uni : isWhite;
...
Range chomp(Range)(Range str)
    if (isRandomAccessRange!Range && isSomeChar!(ElementEncodingType!Range) ||
        isSomeString!Range)
{
    import mystd.uni : lineSep, paraSep, nelSep;
    if (str.empty)
        return str;
    ...
    if (isSomeChar!C1 && isSomeChar!C2)
{
    static if (is(Unqual!C1 == Unqual!C2))
    {
        import mystd.algorithm : startsWith;
...
{
    import mystd.uni : lineSep, paraSep, nelSep;
    import mystd.utf : codeUnitLimit, decodeFront;
...
in
{
    import mystd.ascii : isASCII;
    assert(from.length == to.length);
    assert(from.length <= 256);
    foreach (char c; from)
        assert(mystd.ascii.isASCII(c));
    foreach (char c; to)
        assert(mystd.ascii.isASCII(c));
}
body
...
@safe pure @nogc unittest
{
    ...
    debug(string) trustedPrintf("mystd.string.inPattern.unittest\n");
...
@safe pure @nogc unittest
{
    ...
    debug(string) trustedPrintf("mystd.string.count.unittest\n");
...
@safe pure unittest
{
    ...
    debug(string) trustedPrintf("mystd.string.removechars.unittest\n");
...
@trusted pure unittest
{
    ...
    debug(string) trustedPrintf("mystd.string.squeeze.unittest\n");
...
{
    import mystd.ascii : isAlphaNum;

    if (s.length && mystd.ascii.isAlphaNum(s[$ - 1]))
    {
        auto r = s.dup;
        size_t i = r.length - 1;
...
@safe pure unittest
{
    ...
    debug(string) trustedPrintf("mystd.string.succ.unittest\n");
       ...
       (C1[] str, const(C2)[] from, const(C3)[] to, const(C4)[] modifiers = null)
{
    ...
    foreach (dchar c; str)
    {
        ...
        {
            dchar f = mystd.utf.decode(from, i);
            if (f == '-' && lastf != dchar.init && i < from.length)
            {
                dchar nextf = mystd.utf.decode(from, i);
                if (lastf <= c && c <= nextf)
                {
                    n += c - lastf - 1;
                    if (mod_c)
      ...
      Lfound:
        ...
        {   dchar t = mystd.utf.decode(to, i);
            if (t == '-' && lastt != dchar.init && i < to.length)
            {
                nextt = mystd.utf.decode(to, i);
                n -= nextt - lastt;
                if (n < 0)
                {
                    newc = nextt + n + 1;
...
unittest
{
    ...
    debug(string) trustedPrintf("mystd.string.tr.unittest\n");
    import mystd.algorithm : equal;
 ...
 *
 *    auto abbrevs = mystd.string.abbrev(list);
 *
...
string[string] abbrev(string[] values) @safe pure
{
    ...
    for (size_t i = 0; i < values_length; i = nexti)
    {
        ...
        for (size_t j = 0; j < value.length; j += mystd.utf.stride(value, j))
        {
            string v = value[0 .. j];
