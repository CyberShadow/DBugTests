...
import mystd.range;      // isInputRange, isOutputRange, isForwardRange, ElementType, hasLength
import mystd.traits;     // isArray

version(unittest) import mystd.algorithm, mystd.conv, mystd.file, mystd.stdio;
...
unittest
{
    ...
    { // Encoder and Decoder
        {
            mystd.file.write("testingEncoder", "\nf\nfo\nfoo\nfoob\nfooba\nfoobar");
            ...
            scope(exit)
            {
                f.close();
                assert(!f.isOpen);
                mystd.file.remove("testingEncoder");
            }
        ...
        {
            mystd.file.write("testingDecoder", "\nZg==\nZm8=\nZm9v\nZm9vYg==\nZm9vYmE=\nZm9vYmFy");
            ...
            scope(exit)
            {
                f.close();
                assert(!f.isOpen);
                mystd.file.remove("testingDecoder");
            }
