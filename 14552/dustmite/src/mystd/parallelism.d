/**
$(D mystd._parallelism) implements high-level primitives for SMP _parallelism.
These include parallel foreach, parallel reduce, parallel eager map, pipelining
and future/promise _parallelism.  $(D mystd._parallelism) is recommended when the
...
$(D mystd._parallelism) is based on the concept of a $(D Task).  A $(D Task) is an
...
---
import mystd.algorithm, mystd.parallelism, mystd.range;

void main() {
    // Parallel reduce can be combined with
    // mystd.algorithm.map to interesting effect.
    // The following example (thanks to Russel Winder)
    // calculates pi by quadrature  using
    // mystd.algorithm.map and TaskPool.reduce.
    ...
    // mystd.algorithm.reduce:  24.065 s
    ...
    immutable pi = 4.0 * taskPool.reduce!"a + b"(
        mystd.algorithm.map!getTerm(iota(n))
    );
}
---
...
// Should be private, but mystd.algorithm.reduce is used in the zero-thread case
...
struct Task(alias fun, Args...)
{
    ...
    */
    ...
    static if(__traits(isSame, fun, run))
    {
        ...
        {
            // BUG:  Should check this for delegates too, but mystd.traits
...
// of an alias to represent mystd.file.read.
import mystd.file;
...
1.  It distinguishes mystd.parallelism threads from other threads so that
    the mystd.parallelism daemon threads can be terminated.
...
public:
    ...
    */
    ...
    template map(functions...)
    {
        ...
        auto dupedLines = mystd.algorithm.map!"a.idup"(lineRange);
        ...
        */
        ...
        if(isInputRange!S)
        {
            ...
            public:
                ...
                static if(mystd.range.isInfinite!S)
                {
    ...
    void main()
    {
        ...
        auto duped = mystd.algorithm.map!"a.idup"(lines);
    ...
    */
    ...
    template reduce(functions...)
    {
        ...
        // Using mystd.algorithm.reduce instead:  181 milliseconds
        auto nums = iota(10_000_000.0f);
        auto sumSquares = taskPool.reduce!"a + b"(
            0.0, mystd.algorithm.map!"a * a"(nums)
        );
        ---
            ...
            else
            {
                ...
                if(!ptr)
                {
                    throw new OutOfMemoryError(
                        "Out of memory in mystd.parallelism."
                    );
                }
    ...
    import mystd.conv, mystd.range, mystd.numeric, mystd.stdio;
...
)
{
    ...
    else
    {
        ...
        if(!ptr) throw new OutOfMemoryError("Out of memory in mystd.parallelism.");
...
unittest
{
    ...
    auto indexed = mystd.range.indexed(toIndex, [3, 1, 4, 0, 2]);
    ...
    immutable serialAns = mystd.algorithm.reduce!"a + b"(iota(1000));
    ...
    assert(equal(
               ...
               mystd.algorithm.map!"a * a"(iota(30_000_001))
           ));
    ...
    assert(
        ...
        reduce!"a + b"(0UL,
                       mystd.algorithm.map!"a * a"(iota(3_000_001))
                      )
    );
    ...
    {
        import mystd.file : deleteme;

        string temp_file = mystd.file.deleteme ~ "-tempDelMe.txt";
    ...
    auto myTask = task!(mystd.math.abs)(-1);
    taskPool.put(myTask);
