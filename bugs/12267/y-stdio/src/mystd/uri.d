...
debug(uri) private import mystd.stdio;

/* ====================== URI Functions ================ */

private import mystd.ascii;
private import mystd.c.stdlib;
...
{
    auto s = mystd.utf.toUTF32(uriComponent);
...
size_t uriLength(string s)
{
    ...
    if (s.length > 7 && mystd.string.icmp(s[0 .. 7], "http://") == 0) {
        i = 7;
    }
    else
    {
        if (s.length > 8 && mystd.string.icmp(s[0 .. 8], "https://") == 0)
            i = 8;
        else
            return -1;
    }
