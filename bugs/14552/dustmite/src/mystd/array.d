...
static import mystd.algorithm; // FIXME, remove with alias of splitter

import mystd.range.primitives;
public import mystd.range.primitives : save, empty, popFront, popBack, front, back;
...
Alias for $(XREF algorithm, _splitter).
 +/
deprecated("Please use mystd.algorithm.iteration.splitter instead.")
alias splitter = mystd.algorithm.iteration.splitter;
    ...
    isForwardRange!R2 && is(typeof(appender!(E[])().put(to[0 .. 1]))))
{
    ...
    static if (isSomeString!(E[]))
    {
        import mystd.string : lastIndexOf;
