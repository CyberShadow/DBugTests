...
version (unittest)
{
    ...
    {
        import mystd.process : thisProcessID;
...
version (Posix) void[] read(in char[] name, size_t upTo = size_t.max) @trusted
{
    ...
    import mystd.array : uninitializedArray;
...
version(Posix) unittest
{
    ...
    mystd.process.executeShell(link_cmd~deleteme~"/a/b/c "~deleteme~"/link");
    rmdirRecurse(deleteme);
    enforce(!exists(deleteme));
}
...
{
    import mystd.array : Appender, appender;
    SpanMode _mode;
...
/// Duplicate functionality of D1's $(D mystd.file.listdir()):
unittest
{
    string[] listdir(string pathname)
    {
        ...
        return mystd.file.dirEntries(pathname, SpanMode.shallow)
            .filter!(a => a.isFile)
            .map!(a => mystd.path.baseName(a.name))
            .array;
    }
...
unittest
{
    import mystd.algorithm;
    import mystd.range;
    import mystd.process;
    version(Android)
        string testdir = deleteme; // This has to be an absolute path when
                                   // called from a shared library on Android,
                                   // ie an apk
    else
        string testdir = "deleteme.dmd.unittest.mystd.file" ~ to!string(thisProcessID); // needs to be relative
...
string tempDir() @trusted
{
    ...
    if (cache is null)
    {
        ...
        {
            import mystd.process : environment;
