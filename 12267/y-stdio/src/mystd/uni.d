...
import mystd.traits, mystd.range, mystd.algorithm, mystd.conv,
    mystd.typetuple, mystd.exception, core.stdc.stdlib;
import mystd.array; //@@BUG UFCS doesn't work with 'local' imports
import core.bitop;
...
{
    foreach(v; 0..0x80)
        assert(mystd.ascii.isLower(v) == isLower(v));
    assert(isLower('я'));
    assert(isLower('й'));
    assert(!isLower('Ж'));
    // Greek HETA
