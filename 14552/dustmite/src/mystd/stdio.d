...
is $(D_PARAM public)ally imported when importing $(B mystd.stdio).
...
import mystd.typecons;// Flag
import mystd.stdiobase;
import core.stdc.stddef;// wchar_t
import mystd.range.primitives;// empty, front, isBidirectionalRange
import mystd.traits;// Unqual, isSomeChar, isSomeString
...
public:
    ...
    void popFront()
    {
        ...
        import mystd.format : formattedRead;
        import mystd.string : chomp;
...
struct File
{
    import mystd.range.primitives : ElementEncodingType;
    import mystd.traits : isScalarType, isArray;
 ...
 */
    ...
    {
        import mystd.conv : text;
        import mystd.exception : errnoEnforce;
 ...
 */
    ...
    {
        import mystd.internal.cstring : tempCString;
        import mystd.exception : errnoEnforce;
 ...
 */
    ...
    {
        // Issue 12349
        static import mystd.file;
        auto deleteme = testFilename();
        auto f = File(deleteme, "w");
        scope(exit) mystd.file.remove(deleteme);
 ...
 */
    void flush() @trusted
    {
        import mystd.exception : enforce, errnoEnforce;
 ...
 */
    ...
    {
        import mystd.exception : Exception, errnoEnforce;
 ...
 */
    ...
    unittest
    {
        ...
        f.rawWrite("\r\n\n\r\n");
        f.close();
        assert(mystd.file.read(deleteme) == "\r\n\n\r\n");
    }
 ...
 */
    ...
    unittest
    {
        ...
        auto deleteme = testFilename();
        auto f = File(deleteme, "w+");
        scope(exit) { f.close(); mystd.file.remove(deleteme); }
 ...
 */
    ...
    unittest
    {
        ...
        auto deleteme = testFilename();
        mystd.file.write(deleteme, "abcdefghijklmnopqrstuvwqxyz");
        scope(exit) { mystd.file.remove(deleteme); }
...
*/
    void write(S...)(S args)
    {
        import mystd.traits : isBoolean, isIntegral, isAggregateType;
        ...
        foreach (arg; args)
        {
            ...
            {
                import mystd.format : formattedWrite;

                mystd.format.formattedWrite(w, "%s", arg);
            }
            else static if (isSomeString!A)
            {
                import mystd.range.primitives : put;
                put(w, arg);
            }
            else static if (isIntegral!A)
            {
                import mystd.conv : toTextRange;
...
*/
    void writef(Char, A...)(in Char[] fmt, A args)
    {
        ...
        mystd.format.formattedWrite(lockingTextWriter(), fmt, args);
    }
...
*/
    void writefln(Char, A...)(in Char[] fmt, A args)
    {
        ...
        auto w = lockingTextWriter();
        mystd.format.formattedWrite(w, fmt, args);
        w.put('\n');
    }
...
*/
    ...
    unittest
    {
        ...
        auto deleteme = testFilename();
        mystd.file.write(deleteme, "hello\nworld\n");
        scope(exit) mystd.file.remove(deleteme);
    ...
    unittest
    {
        ...
        auto deleteme = testFilename();
        mystd.file.write(deleteme, "cześć \U0002000D");
        scope(exit) mystd.file.remove(deleteme);
        uint[] lengths = [12,8,7];
...
/** ditto */
    ...
    {
        import mystd.algorithm : endsWith, swap;
        import mystd.range.primitives : back;
...
See_Also:
  ...
  */
    ...
    {
        import mystd.exception : ErrnoException;
        import mystd.file : isSymlink;
        import mystd.path : absolutePath, baseName, buildPath, dirName;

        static string genTempName(const(char)[] prefix, const(char)[] suffix, const(char)[] dir)
        {
            import mystd.ascii : digits, letters;
            import mystd.random : rndGen;
        ...
        for (int i = 0; true; ++i)
        {
            ...
            version (unittest)
            {
                ...
                {
                    import mystd.file;
                    mystd.file.write(filename, "nothing");
                }
            }
            ...
            if (fd == -1)
            {
                import mystd.string: format;
                version (unittest)
                {
                    import mystd.file;
                    if (i == 0 && filename.exists)
                    {
                        mystd.file.remove(filename);
    ...
    {
        import mystd.file : tempDir;
        return File.scratchFile(prefix, suffix, mystd.file.tempDir());
    }

    unittest
    {
        import mystd.algorithm, mystd.conv, mystd.file, mystd.path, mystd.process;

        {
            string name;
            scope(exit) if(name.exists) mystd.file.remove(name);
...
*/
    ...
    private:
         ...
         * when accessing range.front, then using mystd.range.take,
    ...
    private:
        ...
        public:
            ...
            void popFront()
            {
                import mystd.algorithm : endsWith;
                assert(file.isOpen);
                line = buffer;
                ...
                if (line.empty)
                {
                    ...
                    line = null;
                        ...
                        && mystd.algorithm.endsWith(line, terminator))
                {
                    static if (isScalarType!Terminator)
                        enum tlen = 1;
...
Example:
----
import mystd.algorithm, mystd.stdio, mystd.string;
...
Example:
----
import mystd.range, mystd.stdio;
...
/// ditto
    ...
    {
        auto deleteme = testFilename();
        mystd.file.write(deleteme, "hi");
        scope(success) mystd.file.remove(deleteme);
...
Example:
----
import mystd.algorithm, mystd.array, mystd.stdio;
...
/// ditto
    ...
    unittest
    {
        ...
        {
            import mystd.conv : text;
            import mystd.algorithm : sort;
            import mystd.range.primitives : walkLength;

            uint i;
            mystd.file.write(deleteme, txt);
            auto f = File(deleteme);
            scope(exit)
            {
                f.close();
...
import mystd.algorithm, mystd.stdio;
void main()
{
 ...
 */
    ...
    {
    private:
        import mystd.range.primitives : ElementType, isInfinite, isInputRange;
    ...
    public:
        ...
        {
            import mystd.traits : ParameterTypeTuple;
...
/// Get the size of the file, ulong.max if file is not searchable, but still throws if an actual error occurs.
    ...
    {
        import mystd.exception : collectException;
...
unittest
{
    ...
    auto deleteme = testFilename();
    scope(exit) collectException(mystd.file.remove(deleteme));
    mystd.file.write(deleteme, "1 2 3");
    auto f = File(deleteme);
    assert(f.size == 5);
    assert(f.tell == 0);
}
...
struct LockingTextReader
{
    ...
    {
        import mystd.utf : stride, UTFException;
        {
...
{
    static import mystd.file;
    import mystd.range.primitives : isInputRange;
...
unittest // bugzilla 13686
{
    ...
    auto deleteme = testFilename();
    mystd.file.write(deleteme, "Тест");
    scope(exit) mystd.file.remove(deleteme);
...
unittest
{
    ...
    f.close();
    scope(exit) { mystd.file.remove(deleteme); }
    assert(cast(char[]) mystd.file.read(deleteme) == "Hello, world number 42!");
    ...
    //stdout.close();
    // auto result = cast(char[]) mystd.file.read(file);
...
{
    import mystd.traits : isAggregateType;
...
unittest
{
    ...
    f.close();
    version (Windows)
        assert(cast(char[]) mystd.file.read(deleteme) ==
...
unittest
{
    ...
    f.close();
    assert(cast(char[]) mystd.file.read(deleteme) ==  "Hello, nice world number 42!");
    // test write on stdout
    auto saveStdout = stdout;
    scope(exit) stdout = saveStdout;
    stdout.open(deleteme, "w");
...
unittest
{
    ...
    version (Windows)
        ...
        assert(cast(char[]) mystd.file.read(deleteme) ==
                "Hello, nice world number 42!\n",
                cast(char[]) mystd.file.read(deleteme));
    // test write on stdout
    // auto saveStdout = stdout;
    // scope(exit) stdout = saveStdout;
    // stdout.open(file, "w");
    ...
    // }
    // stdout.close();
    // auto read = cast(char[]) mystd.file.read(file);
    // version (Windows)
...
private FILE* fopen(in char[] name, in char[] mode = "r") @trusted nothrow @nogc
{
    ...
    version(Windows)
    {
        import mystd.internal.cstring : tempCStringW;
...
//         }
    ...
    {
        import mystd.exception : assumeUnique;
        import mystd.conv : to;
        import mystd.traits : ParameterTypeTuple;
...
unittest
{
    ...
    foreach (T; TestedWith) {
        ...
        mystd.file.write(deleteme, "Line one\nline two\nline three\n");
        f.open(deleteme, "r");
        uint i = 0;
        foreach (T line; lines(f))
        {
...
__gshared
{
    ...
    unittest
    {
        ...
        import mystd.stdio, mystd.array, mystd.algorithm : sort, copy;
...
private size_t readlnImpl(FILE* fps, ref char[] buf, dchar terminator, File.Orientation /*ignored*/)
{
    ...
    import mystd.array : appender, uninitializedArray;
        ...
        else version (Posix)
        {
            ...
            for (int c; (c = FGETWC(fp)) != -1; )
            {
                if ((c & ~0x7F) == 0)
                    ...
                    mystd.utf.encode(buf, cast(dchar)c);
                if (c == terminator)
                    break;
            }
            if (ferror(fps))
