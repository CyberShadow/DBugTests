...
/** Helper function that returns a _complex number with the specified
    ...
    Returns:
        ...
        deduced using $(D mystd.traits.CommonType!(R, I)).
*/
...
{
    import mystd.format : FormatSpec;
    ...
    See the $(LINK2 std_format.html, mystd.format) and $(XREF string, format)
    ...
    */
    ...
    unittest
    {
        ...
        // Formatting with mystd.string.format specs: the precision and width
    ...
    {
        import mystd.math : signbit;
        import mystd.format : formatValue;
...
@safe pure nothrow @nogc:
        ...
        if (op == "^^" && isNumeric!R)
    {
        ...
        return typeof(return)(ab * mystd.math.cos(ar), ab * mystd.math.sin(ar));
    }
...
unittest
{
    ...
    auto rec1b = (-1.0) ^^ c1;
    assert(approxEqual(abs(rec1b), mystd.math.exp(-PI * c1.im), EPS));
    ...
    auto rec2b = (-1.0) ^^ c2;
    assert(approxEqual(abs(rec2b), mystd.math.exp(-2 * PI), EPS));
    ...
    foreach (i; 0..6)
    {
        ...
        assert (approxEqual(mystd.math.cos(arg(cei)), mystd.math.cos(arg(c1)*i), EPS));
    }
...
unittest
{
    ...
    assert (abs(complex(1.0L, -2.0L)) == mystd.math.sqrt(5.0L));
}
    ...
    @safe pure nothrow @nogc
{
    return Complex!(CommonType!(T,U))
        (modulus*mystd.math.cos(argument), modulus*mystd.math.sin(argument));
}

unittest
{
    import mystd.math;
    auto z = fromPolar(mystd.math.sqrt(2.0), PI_4);
...
{
    import mystd.math : expi, coshisinh;
    auto cs = expi(z.re);
...
unittest
{
  ...
  assert(sin(complex(2.0L, 0)) == mystd.math.sin(2.0L));
}
...
unittest{
    ...
    assert(cos(complex(1.3L)) == mystd.math.cos(1.3L));
...
{
    import mystd.math : cos, sin;
    return Complex!real(cos(y), sin(y));
}

unittest
{
    assert(expi(1.3e5L) == complex(mystd.math.cos(1.3e5L), mystd.math.sin(1.3e5L)));
    assert(expi(0.0L) == 1.0L);
    auto z1 = expi(1.234);
    auto z2 = mystd.math.expi(1.234);
...
Complex!T sqrt(T)(Complex!T z)  @safe pure nothrow @nogc
{
    ...
    else
    {
        ...
        if (x >= y)
        {
            r = y / x;
            w = mystd.math.sqrt(x)
                * mystd.math.sqrt(0.5 * (1 + mystd.math.sqrt(1 + r * r)));
        }
        else
        {
            r = x / y;
            w = mystd.math.sqrt(y)
                * mystd.math.sqrt(0.5 * (r + mystd.math.sqrt(1 + r * r)));
        }
...
unittest
{
    ...
    assert (sqrt(complex(1.0L, 0)) == mystd.math.sqrt(1.0L));
