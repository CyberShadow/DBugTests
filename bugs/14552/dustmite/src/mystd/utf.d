...
import mystd.traits;     // isSomeChar, isSomeString
...
/++
    Exception thrown on errors in mystd.utf functions.
  +/
...
particularly, in $(D mystd.string).
  +/
...
auto byChar(R)(R r) if (isNarrowString!R)
{
     ...
     * won't get auto-decoded by mystd.array.front().
     */
    ...
    if (!isNarrowString!R && isInputRange!R && isSomeChar!(ElementEncodingType!R))
{
    ...
    /* Defeat the auto-decoding of mystd.array.put() by handling arrays of chars
     * explicitly.
     */
    static if (is(tchar == char))
    {
