...
public import mystd.container.util;
...
When using $(D Array) with range-based functions like those in $(D mystd.algorithm),
...
if (!is(Unqual!T == bool))
{
    ...
    import mystd.algorithm : initializeAll, copy;
    import mystd.exception : enforce;
    import mystd.typecons : RefCounted, RefCountedAutoInitialize;
...
Complexity: $(BIGOH n + m), where $(D m) is the length of $(D stuff)
    ...
    if (isInputRange!Stuff && isImplicitlyConvertible!(ElementType!Stuff, T))
    {
        ...
        {
            import mystd.algorithm : bringToFront;
            enforce(_data);
...
unittest
{
    ...
    import mystd.container : Array, BinaryHeap;
