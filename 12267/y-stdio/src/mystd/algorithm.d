 ...
 */
module mystd.algorithm;
//debug = std_algorithm;

import mystd.functional : unaryFun, binaryFun;
import mystd.range;
import mystd.traits;
import mystd.typecons : tuple, Tuple;
import mystd.typetuple : TypeTuple, staticMap, allSatisfy;

version(unittest)
{
    debug(std_algorithm) import mystd.stdio;
    mixin(dummyRanges);
}
...
template map(fun...) if (fun.length >= 1)
{
    auto map(Range)(Range r) if (isInputRange!(Unqual!Range))
    {
        ...
        {
            import mystd.functional : adjoin;
            import mystd.typetuple : staticIndexOf;
...
unittest
{
    ...
    auto ms1 = map!(mystd.ascii.toUpper)(s1);
    auto ms2 = map!(mystd.ascii.toUpper)(s2);
    auto ms3 = map!(mystd.ascii.toUpper)(s3);
...
unittest
{
    import mystd.math : approxEqual, sqrt;
...
unittest
{
    import mystd.functional : compose, pipe;
...
void move(T)(ref T source, ref T target)
{
    ...
    import mystd.exception : pointsTo;
...
unittest
{
    ...
    //@@@6791@@@
    assert(equal(mystd.array.splitter("là dove terminava quella valle"), ["là", "dove", "terminava", "quella", "valle"]));
    assert(equal(splitter!(mystd.uni.isWhite)("là dove terminava quella valle"), ["là", "dove", "terminava", "quella", "valle"]));
...
$(D splitter!(mystd.uni.isWhite)), runs of whitespace will be merged together
...
if (isSomeChar!C)
{
    ...
    private:
        ...
        {
            auto r = find!(mystd.uni.isWhite)(_s);
...
unittest
{
    ...
    foreach (line; lines) {
       foreach (word; mystd.array.splitter(mystd.string.strip(line))) {
...
{
    import mystd.conv : text;
    import mystd.string : split, format;
...
unittest
{
    ...
    assert(equal(joiner(["Mary", "has", "a", "little", "lamb"]),
                    "Maryhasalittlelamb"));
    assert(equal(joiner(mystd.range.repeat("abc", 3)), "abcabcabc"));
    ...
    is (typeof(binaryFun!pred(haystack.front, needle)) : bool))
{
    ...
    static if (isNarrowString!R)
    {
        ...
        static if (isDefaultPred)
        {
            ...
            {
                return cast(R) mystd.algorithm.find(haystack, needle);
            }
...
unittest
{
    import mystd.exception : assertCTFEable;
...
unittest
{
    ...
    int[] a = [ 1, 2, 3, 2, 6 ];
    assert(find(mystd.range.retro(a), 5).empty);
    assert(equal(find(mystd.range.retro(a), 2), [ 2, 3, 2, 1 ][]));
    ...
    foreach (T; TypeTuple!(int, double))
    {
        ...
        b[$ / 2] = 200;
        b[$ / 4] = 200;
        assert(find(mystd.range.retro(b), 200).length ==
...
unittest
{
    import mystd.ascii : isDigit;
    import mystd.uni : isWhite;

    assert(countUntil!(mystd.uni.isWhite)("hello world") == 5);
    assert(countUntil!(mystd.ascii.isDigit)("hello world") == -1);
...
if (isNarrowString!R1 && isNarrowString!R2)
{
    static if (ElementEncodingType!R1.sizeof == ElementEncodingType!R2.sizeof)
    {
        ...
        {
            immutable codeLen = mystd.utf.stride(r1, i);
            size_t j = 0;
...
unittest
{
    ...
    assert(count!((a, b) => mystd.uni.toLower(a) == mystd.uni.toLower(b))("AbcAdFaBf", "ab") == 2);
...
{
    import mystd.algorithm : equal;
    import mystd.range : iota, chunks;
...
int cmp(alias pred = "a < b", R1, R2)(R1 r1, R2 r2) if (isSomeString!R1 && isSomeString!R2)
{
    ...
    else
    {
        for (size_t i1, i2;;)
        {
            ...
            immutable c1 = mystd.utf.decode(r1, i1),
                c2 = mystd.utf.decode(r2, i2);
...
unittest
{
    import mystd.uni : toUpper;

    assert(levenshteinDistance("cat", "rat") == 1);
    assert(levenshteinDistance("parks", "spark") == 2);
    assert(levenshteinDistance("kitten", "sitting") == 3);
    assert(levenshteinDistance!((a, b) => mystd.uni.toUpper(a) == mystd.uni.toUpper(b))
        ("parks", "SPARK") == 2);
}
...
unittest
{
    import mystd.algorithm, mystd.range;
    int[] src = [1, 2, 4];
    int[] dst = [0, 0, 0, 0, 0];
...
if (isNarrowString!(Char[]) && !is(Char == const) && !is(Char == immutable))
{
    ...
    import mystd.utf : stride;

    auto r = representation(s);
    for (size_t i = 0; i < s.length; )
    {
        immutable step = mystd.utf.stride(s, i);
        if (step > 1)
        {
            .reverse(r[i .. i + step]);
            i += step;
...
{
    import mystd.conv : text;
    import mystd.random : Random, unpredictableSeed, uniform;
...
mystd.algorithm) only change $(I content), not $(I topology). The value
...
{
    import mystd.container : BinaryHeap;
...
unittest
{
    import mystd.random : Random, uniform;
...
unittest
{
    import mystd.random : Random, uniform, randomShuffle;
    ...
    SRange, TRange)(SRange source, TRange target)
{
    ...
    alias TElem = mystd.iterator.ElementType!(TRange);
...
unittest
{
    ...
    partialIndex!((a, b) => mystd.uni.toUpper(a) < mystd.uni.toUpper(b))(b, index);
    assert(isSorted!((a, b) => mystd.uni.toUpper(*a) < mystd.uni.toUpper(*b))(index));

    // random data with indexes
    auto index1 = new size_t[b.length];
    bool cmp(string x, string y) { return mystd.uni.toUpper(x) < mystd.uni.toUpper(y); }
    partialIndex!(cmp)(b, index1);
    bool check(size_t x, size_t y) { return mystd.uni.toUpper(b[x]) < mystd.uni.toUpper(b[y]); }
...
unittest
{
    import mystd.random : Random, unpredictableSeed, uniform, randomShuffle;
    ...
    if (Values.length != 0)
{
    ...
    {
        import mystd.functional : binaryFun;
