...
private {
  ...
  import mystd.ascii;
  import mystd.format;
  import mystd.system;    // for Endian enumeration
  import mystd.utf;
...
interface InputStream {
   ...
   * and <a href="std_format.html">mystd.format</a>.
   *
...
interface OutputStream {
   ...
   * References: <a href="std_format.html">mystd.format</a>.
...
class Stream : InputStream, OutputStream {
  private import mystd.string, mystd.digest.crc, core.stdc.stdlib, core.stdc.stdio;
  ...
  size_t vprintf(const(char)[] format, va_list args) {
    ...
    // "cannot mix core.mystd.stdlib.alloca() and exception handling".
  ...
  unittest { // unit test for Issue 3363
    import mystd.stdio;
    immutable fileName = mystd.file.deleteme ~ "-issue3363.txt";
...
class File: Stream {
  ...
  unittest {
    ...
    File file = new File;
    int i = 666;
    auto stream_file = mystd.file.deleteme ~ "-stream.$$$";
    ...
    assert(!mystd.algorithm.cmp(line, "Testing stream.d:"));
    // jump over "Hello, "
    ...
    assert(!mystd.algorithm.cmp(file.readString(6), "world!"));
    i = 0; file.read(i);
    assert(i == 666);
    // string#1 + string#2 + int should give exacly that
    version (Windows)
...
class BufferedFile: BufferedStream {
  ...
  unittest {
    ...
    assert(!mystd.algorithm.cmp(file.readLine(), "Testing stream.d:"));
    // jump over "Hello, "
    file.seek(7, SeekPos.Current);
    version (Windows)
      assert(file.position == 19 + 7);
...
class EndianStream : FilterStream {
   ...
   * in the mystd.system module.
   */
  this(Stream source, Endian end = mystd.system.endian) {
    super(source);
    endian = end;
  }
  ...
  unittest {
    ...
    em.fixBO(x3.ptr,12);
    if (mystd.system.endian == Endian.littleEndian) {
      assert( x3[0] == 12 );
      assert( x3[1] == 11 );
      assert( x3[2] == 10 );
      assert( x3[4] == 8 );
    ...
    em.position = 0;
    em.fixBO(x3.ptr,12);
    if (mystd.system.endian == Endian.bigEndian) {
      assert( x3[0] == 12 );
      assert( x3[1] == 11 );
      assert( x3[2] == 10 );
      assert( x3[4] == 8 );
...
unittest {
  auto test_file = mystd.file.deleteme ~ "-testing.txt";
