...
/++
  ...
  For an introduction to $(D mystd.regex) see a
  ...
  $(I mystd.regex operates on codepoint level,
  ...
  *With exception of point 1.1.1, as of yet, normalization of input
  ...
  Authors: Dmitry Olshansky,
    ...
    API and utility constructs are modeled after the original $(D mystd.regex)
...
import mystd.internal.uni, mystd.internal.uni_tab;//Unicode property tables
import mystd.array, mystd.algorithm, mystd.range,
       mystd.conv, mystd.exception, mystd.traits, mystd.typetuple,
       mystd.utf, mystd.format, mystd.typecons, mystd.bitmanip,
       mystd.functional, mystd.exception;

import core.bitop, core.stdc.string, core.stdc.stdlib;
static import mystd.ascii;
import mystd.string : representation;

debug(std_regex_parser) import mystd.stdio; //trace parser progress
debug(std_regex_search) import mystd.stdio; //trace prefix search engine
debug(std_regex_matcher) import mystd.stdio;  //trace matcher engine
debug(std_regex_allocation) import mystd.stdio; //track object life cycle
debug(std_regex_ctr) import mystd.stdio; //dump ctRegex generated sources
debug(std_regex_test) import mystd.stdio; //trace test suite progress

private:

import mystd.uni : isAlpha, isWhite;
...
{
    //mystd.conv.parse is both @system and bogus
    ...
    if (isForwardRange!R && is(ElementType!R : dchar))
{
    ...
    {
        uint r = 0;
        while(mystd.ascii.isDigit(current))
        {
            if(r >= (uint.max/10))
                ...
                if(current == '?')
                {
                    ...
                    case 'P':
                        ...
                        while(next() && (isAlpha(current) ||
                            current == '_' || mystd.ascii.isDigit(current)))
                        {
                            name ~= current;
                        }
                        if(current != '>')
    ...
    {//copy is @system
        ...
        case '{':
            ...
            enforce(mystd.ascii.isDigit(current), "First number required in repetition");
            min = parseDecimal();
            if(current == '}')
                max = min;
            else if(current == ',')
            {
                next();
                if(mystd.ascii.isDigit(current))
                    max = parseDecimal();
                else if(current == '}')
                    max = infinite;
                else
        ...
        if(max != infinite)
        {
            if(min != 1 || max != 1)
            {
                ...
                putRaw(min);
                putRaw(max);
                counterDepth = mystd.algorithm.max(counterDepth, nesting+1);
            }
        }
    ...
    {//accesses array of appender
        ...
        case '1': .. case '9':
            ...
            while(nref < maxBackref && next() && mystd.ascii.isDigit(current))
            {
        ...
        if(current == '{')
        {
            while(k < MAX_PROPERTY && next() && current !='}' && current !=':')
                if(current != '-' && current != ' ' && current != '_')
                    result[k++] = cast(char)mystd.ascii.toLower(current);
    ...
    if(is(Char :dchar))
{
    ...
    bool nextChar(ref dchar res, ref size_t pos)
    {
        ...
        if(_index == _origin.length)
            return false;
        res = mystd.utf.decode(_origin, _index);
        return true;
    }
    @property bool atEnd(){
    ...
    struct BackLooper
    {
        ...
        @trusted bool nextChar(ref dchar res,ref size_t pos)
        {
            ...
            _index -= mystd.utf.strideBack(_origin, _index);

            return true;
        }
        @property atEnd(){ return _index == 0 || _index == mystd.utf.strideBack(_origin, _index); }
        auto loopBack(size_t index){   return Input(_origin, index); }

        //support for backtracker engine, might not be present
        //void reset(size_t index){   _index = index ? index-mystd.utf.strideBack(_origin, index) : 0;  }
        void reset(size_t index){   _index = index;  }
        ...
        for(;;)
        {
            ...
            case IR.Backref:
                ...
                else static if(withInput)
                {
                    ...
                    {
                        t.uopCounter += mystd.utf.stride(s[idx..end], 0);
...
public:
    ...
    ///A hook for compatibility with original mystd.regex.
...
public:
    ...
    /// Same as .front, provided for compatibility with original mystd.regex.
...
/++
    ...
    {
        return mystd.string.toUpper(m.hit);
    }
...
L_Replace_Loop:
    while(!format.empty)
        ...
        case State.Normal:
            ...
            break;
        case State.Dollar:
            if(mystd.ascii.isDigit(format[0]))
            {
            ...
            {
                auto x = find!(a => !mystd.ascii.isAlpha(a))(format[1..$]);
...
//tests for accumulated mystd.regex issues and other regressions
unittest
{
    void test_body(alias matchFn)()
    {
...
unittest
{
    void test(alias matchFn)()
    {
        ...
        foreach(i, v; TypeTuple!(string, wstring, dstring))
        {
            ...
            alias String = v;
            assert(mystd.regex.replace!(matchFn)(to!String("ark rapacity"), regex(to!String("r")), to!String("c"))
                   == to!String("ack rapacity"));
            assert(mystd.regex.replace!(matchFn)(to!String("ark rapacity"), regex(to!String("r"), "g"), to!String("c"))
                   == to!String("ack capacity"));
            assert(mystd.regex.replace!(matchFn)(to!String("noon"), regex(to!String("^n")), to!String("[$&]"))
                   == to!String("[n]oon"));
            assert(mystd.regex.replace!(matchFn)(to!String("test1 test2"), regex(to!String(`\w+`),"g"), to!String("$`:$'"))
                   == to!String(": test2 test1 :"));
            auto s = mystd.regex.replace!(baz!(Captures!(String)))(to!String("Strap a rocket engine on a chicken."),
...
{// bugzilla 7679
    foreach(S; TypeTuple!(string, wstring, dstring))
    {
        ...
        auto str = to!S("a.b");
        assert(equal(mystd.regex.splitter(str, re), [to!S("a"), to!S("b")]));
