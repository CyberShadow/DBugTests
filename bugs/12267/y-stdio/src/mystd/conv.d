...
import mystd.algorithm, mystd.array, mystd.ascii, mystd.exception, mystd.range,
    mystd.string, mystd.traits, mystd.typecons, mystd.typetuple, mystd.uni,
    mystd.utf;
import mystd.format;
...
@safe pure unittest
{
    ...
    // make sure mystd.conv.to is doing the same thing as initialization
    Test.S s;
    Test.T t = s;
}
        ...
        isFloatingPoint!Target && !is(Target == enum))
{
    ...
    case '-':
        ...
        p.popFront();
        enforce(!p.empty, bailOut());
        if (mystd.ascii.toLower(p.front) == 'i')
            goto case 'i';
        enforce(!p.empty, bailOut());
        break;
    case '+':
    ...
    case 'i': case 'I':
        p.popFront();
        enforce(!p.empty, bailOut());
        if (mystd.ascii.toLower(p.front) == 'n')
        {
            p.popFront();
            enforce(!p.empty, bailOut());
            if (mystd.ascii.toLower(p.front) == 'f')
            {
                // 'inf'
                p.popFront();
    ...
    if (isHex)
    {
        ...
        while (!p.empty)
        {
            ...
            {
                anydigits = 1;
                i = mystd.ascii.isAlpha(i) ? ((i & ~0x20) - ('A' - 10)) : i - '0';
                if (ndigits < 16)
                {
                    msdec = msdec * 16 + i;
                    if (msdec)
    ...
    {
        if (mystd.ascii.toUpper(p.front) == 'N' && !startsWithZero)
        {
            // nan
            p.popFront();
            enforce(!p.empty && mystd.ascii.toUpper(p.front) == 'A',
                   new ConvException("error converting input to floating point"));
            p.popFront();
            enforce(!p.empty && mystd.ascii.toUpper(p.front) == 'N',
                   new ConvException("error converting input to floating point"));
...
package void skipWS(R)(ref R r)
{
    ...
    {
        for (; !r.empty && mystd.ascii.isWhite(r.front); r.popFront())
        {}
    }
}
    ...
    if (isInputRange!Source && isSomeChar!(ElementType!Source))
{
    ...
    dchar getHexDigit()(ref Source s_ = s)  // workaround
    {
        ...
        return mystd.ascii.isAlpha(c) ? ((c & ~0x20) - ('A' - 10)) : c - '0';
    }
