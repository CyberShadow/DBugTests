 ...
 *
 * This module conforms to the APIs defined in $(D mystd.digest.digest). To understand the
 * differences between the template and the OOP API, see $(D mystd.digest.digest).
 *
 * This module publicly imports $(D mystd.digest.digest) and can be used as a stand-alone
 * module.
 *
 ...
 */
module mystd.digest.sha;

///
unittest
{
    //Template API
    import mystd.digest.sha;
...
import mystd.ascii : hexDigits;
import mystd.exception : assumeUnique;
import core.bitop : bswap;
version(USE_SSSE3) import core.cpuid : hasSSSE3Support = ssse3;
version(USE_SSSE3) import mystd.internal.digest.sha_SSSE3 : transformSSSE3;
...
public import mystd.digest.digest;

/*
 * Helper methods for encoding the buffer.
 * Can be removed if the optimizer can inline the methods from mystd.bitmanip.
 */
 ...
 * See $(D mystd.digest.digest) for differences between template and OOP API.
 */
struct SHA1
{
    version(USE_SSSE3)
